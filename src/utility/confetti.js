export const Confetti = function(ell){
// utilities
var _ = {};
...
// some constants
const DECAY = 10;        // confetti decay in seconds
const SPREAD = 220;      // degrees to spread from the angle of the cannon
const GRAVITY = 800;
...
class ConfettiCannon {
    ...
    tweenConfettiParticle(id) {
        const minAngle = this.confettiSprites[id].angle - SPREAD / 3;
        const maxAngle = this.confettiSprites[id].angle + SPREAD / 1;

        const minVelocity = this.confettiSprites[id].velocity / 2;
        const maxVelocity = this.confettiSprites[id].velocity * 2;
        ...
        TweenLite.to(this.confettiSprites[id], DECAY, {
            ...
            onComplete: () => {
                ...
                this.confettiSpriteIds = this.confettiSpriteIds.filter( (ids)=>{ return ids !== id} )
    ...
    drawPower() {
        ...
        this.ctx.beginPath();
        this.ctx.arc(centerX, centerY, radius, 0, 3 * Math.PI, false);
        this.ctx.fillStyle = 'transparent';
        this.ctx.fill();
        this.ctx.lineWidth = 2 * this.dpr;
...
const confetti = new ConfettiCannon(ell);
 confetti.handleMouseup({x0:window.innerWidth/ 2-40, y0: window.innerHeight/2 , y1:window.innerHeight/2 +150, x1:window.innerWidth/2});
 confetti.handleMouseup({x0:window.innerWidth/ 2+20,  y0: window.innerHeight/2 , y1:window.innerHeight/2 +150, x1:window.innerWidth/ 2})
}
